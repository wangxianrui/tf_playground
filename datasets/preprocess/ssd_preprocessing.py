...
www.robots.ox.ac.uk/~vggnet/research/very_deep/
"""
...
def ssd_random_sample_patch(image,
    ...
    def sample_width_height(width, height):
        with tf.name_scope('sample_width_height'):
            ...
            return tf.cast(sampled_width, tf.int32), tf.cast(
                sampled_height, tf.int32)
    ...
    def check_roi_center(width, height, labels, bboxes):
        with tf.name_scope('check_roi_center'):
            ...
            def condition(index, roi, mask):
                return tf.logical_or(
                    tf.logical_and(
                        tf.reduce_sum(tf.cast(mask, tf.int32)) < 1,
                        tf.less(index, max_attempt)), tf.less(index, 1))
            ...
            def body(index, roi, mask):
                ...
                roi = [
                    tf.cast(y, tf.float32) / float_height,
                ...
                mask_min = tf.logical_and(
                    tf.greater(center_y, roi[0]), tf.greater(center_x, roi[1]))
                mask_max = tf.logical_and(
                    tf.less(center_y, roi[2]), tf.less(center_x, roi[3]))
                mask = tf.logical_and(mask_min, mask_max)

                return index + 1, roi, mask

            [index, roi, mask] = tf.while_loop(
                condition,
                body, [index, roi, mask],
    ...
    def check_roi_overlap(width, height, labels, bboxes, min_iou):
        with tf.name_scope('check_roi_overlap'):
                ...
                return tf.logical_or(
                    tf.logical_or(
                        tf.logical_and(
                            tf.reduce_sum(
                                tf.cast(
                                    jaccard_with_anchors(roi, mask_bboxes) <
                                    min_iou, tf.int32)) > 0,
                            tf.less(index, max_attempt)), tf.less(index, 1)),
            ...
            [index, roi, mask_labels, mask_bboxes] = tf.while_loop(
                condition,
            ...
            return tf.cond(
                tf.greater(tf.shape(mask_labels)[0], 0), lambda: (tf.cast([
                    roi[0] * tf.cast(height, tf.float32), roi[1] * tf.cast(
                        width, tf.float32), (roi[2] - roi[0]) * tf.cast(
                            height, tf.float32), (roi[3] - roi[1]) * tf.cast(
                                width, tf.float32)
                ], tf.int32), mask_labels, mask_bboxes), lambda: (tf.cast(
    ...
    def sample_patch(image, labels, bboxes, min_iou):
        with tf.name_scope('sample_patch'):
            ...
            scale = tf.cast(
                tf.stack([height, width, height, width]), mask_bboxes.dtype)
            ...
            cliped_ymax = tf.minimum(
                tf.cast(roi_slice_range[2], tf.float32), mask_bboxes[:, 2])
            cliped_xmax = tf.minimum(
                tf.cast(roi_slice_range[3], tf.float32), mask_bboxes[:, 3])
            ...
            scale = tf.cast(
                tf.stack([
                    roi_slice_range[2], roi_slice_range[3], roi_slice_range[2],
                    roi_slice_range[3]
                ]), mask_bboxes.dtype)
            ...
            return tf.cond(
                tf.logical_or(
                    ...
                    tf.less(roi_slice_range[3],
                            1)), lambda: (image, labels, bboxes), lambda:
    ...
    with tf.name_scope('ssd_random_sample_patch'):
        ...
        return tf.cond(
            tf.less(sampled_min_iou, 1.), lambda: sample_patch(
                image, labels, bboxes, sampled_min_iou), lambda: (
                    image, labels, bboxes))
...
def ssd_random_expand(image, bboxes, ratio=2., name=None):
    with tf.name_scope('ssd_random_expand'):
        ...
        canvas_width, canvas_height = tf.to_int32(
            float_width * ratio), tf.to_int32(float_height * ratio)

        mean_color_of_image = [
            _R_MEAN / 255., _G_MEAN / 255., _B_MEAN / 255.
        ]  # tf.reduce_mean(tf.reshape(image, [-1, 3]), 0)
        ...
        paddings = tf.convert_to_tensor([[y, canvas_height - height - y],
        ...
        absolute_bboxes = bboxes * scale + tf.cast(
            tf.stack([y, x, y, x]), bboxes.dtype)

        return big_canvas, absolute_bboxes / tf.cast(
            tf.stack([
                canvas_height, canvas_width, canvas_height, canvas_width
            ]), bboxes.dtype)
    ...
    with tf.name_scope('ssd_random_sample_patch_wrapper'):
        ...
        def check_bboxes(bboxes):
            ...
            return tf.logical_and(
                tf.logical_and(areas < 0.9, areas > 0.001),
        ...
        def condition(index, image, labels, bboxes, orgi_image, orgi_labels,
                      orgi_bboxes):
            return tf.logical_or(
                tf.logical_and(
                    tf.reduce_sum(tf.cast(check_bboxes(bboxes), tf.int64)) < 1,
                    tf.less(index, max_attempt)), tf.less(index, 1))

        def body(index, image, labels, bboxes, orgi_image, orgi_labels,
                 orgi_bboxes):
            image, bboxes = tf.cond(
                tf.random_uniform([], minval=0., maxval=1., dtype=tf.float32) <
                0.5, lambda: (
                    orgi_image, orgi_bboxes), lambda: ssd_random_expand(
                        orgi_image, orgi_bboxes,
                        tf.random_uniform(
                            [1], minval=1.1, maxval=4., dtype=tf.float32)[0]))
            # Distort image and bounding boxes.
            random_sample_image, labels, bboxes = ssd_random_sample_patch(
                image,
                orgi_labels,
                bboxes,
                ratio_list=[-0.1, 0.1, 0.3, 0.5, 0.7, 0.9, 1.])
            random_sample_image.set_shape([None, None, 3])
            return index + 1, random_sample_image, labels, bboxes, orgi_image, orgi_labels, orgi_bboxes

        [index, image, labels, bboxes, orgi_image, orgi_labels,
         orgi_bboxes] = tf.while_loop(
             condition,
             body, [
                 index, image, labels, bboxes, orgi_image, orgi_labels,
                 orgi_bboxes
        ...
        valid_mask = check_bboxes(bboxes)
        labels, bboxes = tf.boolean_mask(labels, valid_mask), tf.boolean_mask(
            bboxes, valid_mask)
        return tf.cond(
            tf.less(index, max_attempt), lambda: (image, labels, bboxes),
            lambda: (orgi_image, orgi_labels, orgi_bboxes))
...
def random_flip_left_right(image, bboxes):
    with tf.name_scope('random_flip_left_right'):
        ...
        # Flip image.
        result = tf.cond(mirror_cond, lambda: tf.image.flip_left_right(image),
                         lambda: image)
        # Flip bboxes.
        mirror_bboxes = tf.stack(
...
def preprocess_for_train(image,
    ...
    with tf.name_scope(scope, 'ssd_preprocessing_train',
        ...
        random_sample_image, labels, bboxes = ssd_random_sample_patch_wrapper(
            distort_image, labels, bboxes)
        ...
        random_sample_flip_image, bboxes = random_flip_left_right(
            random_sample_image, bboxes)
        # Rescale to VGG input scale.
        random_sample_flip_resized_image = tf.image.resize_images(
        ...
        final_image = tf.to_float(
            tf.image.convert_image_dtype(
                random_sample_flip_resized_image, orig_dtype, saturate=True))
        final_image = _mean_image_subtraction(final_image,
                                              [_R_MEAN, _G_MEAN, _B_MEAN])

        final_image.set_shape(out_shape + [3])
        if not output_rgb:
            image_channels = tf.unstack(final_image, axis=-1, name='split_rgb')
            final_image = tf.stack(
                [image_channels[2], image_channels[1], image_channels[0]],
                axis=-1,
                name='merge_bgr')
...
def preprocess_image(image,
    ...
    if is_training:
        ...
        return preprocess_for_eval(
            image, out_shape, data_format=data_format, output_rgb=output_rgb)

...
class PeleeNetSSD:
    ...
    def __call__(self, inputs, is_train):
        with tf.variable_scope('backbone'):
            stage3, stage4 = self.peleenet(inputs, is_train)

        with tf.variable_scope('feature_exactor'):
            feature_layers = []
            with tf.variable_scope('extra_pm2'):
                pm2_inputs = stage3
                pm2_res = self.add_extra(pm2_inputs, is_train, self.extra_output_channel)
                feature_layers.append(pm2_res)
                feature_layers.append(pm2_res)

            with tf.variable_scope('extra_pm3'):
                pm3_inputs = stage4
                pm3_res = self.add_extra(pm3_inputs, is_train, self.extra_output_channel)
                feature_layers.append(pm3_res)

            with tf.variable_scope('extra_pm3_to_pm4'):
                pm3_to_pm4 = tf.layers.Conv2D(self.extra_output_channel, 1, 1)(pm3_inputs)
                pm3_to_pm4 = tf.nn.relu(pm3_to_pm4)
                pm3_to_pm4 = tf.layers.Conv2D(self.extra_output_channel, 3, 2, padding='same')(pm3_to_pm4)
                pm3_to_pm4 = tf.nn.relu(pm3_to_pm4)

            with tf.variable_scope('extra_pm4'):
                pm4_inputs = pm3_to_pm4
                pm4_res = self.add_extra(pm4_inputs, is_train, self.extra_output_channel)
                feature_layers.append(pm4_res)

            with tf.variable_scope('extra_pm4_to_pm5'):
                pm4_to_pm5 = tf.layers.Conv2D(self.extra_output_channel, 1, 1)(pm4_inputs)
                pm4_to_pm5 = tf.nn.relu(pm4_to_pm5)
                pm4_to_pm5 = tf.layers.Conv2D(self.extra_output_channel, 3, 1)(pm4_to_pm5)
                pm4_to_pm5 = tf.nn.relu(pm4_to_pm5)

            with tf.variable_scope('extra_pm5'):
                pm5_inputs = pm4_to_pm5
                pm5_res = self.add_extra(pm5_inputs, is_train, self.extra_output_channel)
                feature_layers.append(pm5_res)

            with tf.variable_scope('extra_pm5_to_pm6'):
                pm5_to_pm6 = tf.layers.Conv2D(self.extra_output_channel, 1, 1)(pm5_inputs)
                pm5_to_pm6 = tf.nn.relu(pm5_to_pm6)
                pm5_to_pm6 = tf.layers.Conv2D(self.extra_output_channel, 3, 1)(pm5_to_pm6)
                pm5_to_pm6 = tf.nn.relu(pm5_to_pm6)

            with tf.variable_scope('extra_pm6'):
                pm6_inputs = pm5_to_pm6
                pm6_res = self.add_extra(pm6_inputs, is_train, self.extra_output_channel)
        ...
        with tf.variable_scope('detection'):
            ...
            for i, feature in enumerate(feature_layers):
                locations.append(
                    tf.layers.Conv2D(Param.anchor_depth_per_layer[i] * 4, kernel_size=3, padding='same',
                                     use_bias=True)(feature))
                classes.append(
                    tf.layers.Conv2D(Param.anchor_depth_per_layer[i] * self.num_classes, kernel_size=3,

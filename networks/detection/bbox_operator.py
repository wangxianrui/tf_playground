...
def intersection(bboxes1, bboxes2):
    ...
    # 1 x T
    xmin_2, ymin_2, xmax_2, ymax_2 = tf.split(
        tf.transpose(bboxes2, [1, 0]), 4, axis=0)
...
def bbox_iou(bboxes1, bboxes2):
    ...
    union_vol = areas(bboxes1) + tf.transpose(
        areas(bboxes2), perm=[1, 0]) - inter_vol
    return tf.where(
        tf.equal(union_vol, 0.0), tf.zeros_like(inter_vol),
        inter_vol / union_vol)
...
def bbox_matching(iou_matrix):
    ...
    between_mask = tf.logical_and(
        tf.less(match_values, Param.match_high),
    ...
    match_indices = tf.where(ignore_mask, -2 * tf.ones_like(match_indices),
                             match_indices)
    anchors_to_gt_mask = tf.one_hot(
        tf.clip_by_value(match_indices, -1,
                         tf.cast(tf.shape(iou_matrix)[0], tf.int64)),
    ...
    left_gt_to_anchors_mask = tf.one_hot(
        gt_to_anchors,
        ...
        axis=1,
        dtype=tf.int32)
    left_gt_to_anchors_scores = iou_matrix * tf.to_float(
    ...
    selected_scores = tf.gather_nd(
        ...
        tf.stack([
            tf.where(
                ...
                tf.argmax(left_gt_to_anchors_scores, axis=0), anchors_to_gt),

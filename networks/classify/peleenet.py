...
class PeleeNetClassify:
    ...
    def forward(self, inputs, is_train):
        _, feature = self.peleenet.forward(inputs, is_train)
        ...
        dense = tf.layers.Dense(ClassifyParam.num_classes)(flatten)
        return dense

    def calc_loss(self, logits, labels, trainabel_vars):
        loss = tf.losses.softmax_cross_entropy(labels, logits)
        loss_filter = lambda var: 'batch_norm' not in var.name
        loss += ClassifyParam.weight_decay * tf.add_n([tf.nn.l2_loss(var) for var in trainabel_vars if loss_filter(var)])
        return loss
